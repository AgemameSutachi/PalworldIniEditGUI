...
class GUI:
    ...
    def inicherk(self):
        ...
        if not os.path.exists(self.shutdownExePath):
            messagebox.showerror(
                "設定エラー",
                "シャットダウンexeが見つかりません、設定を見直してください\nファイル名: "
                + Cl_Con.config_path
                + "\nキー名: ShutdownExePath",
            )
            return 1
    ...
    def create_progress_window(self):
        ...
        self.btn_progress_close = ttk.Button(
            self.dlg_progress, text="閉じる", command=self.click_btn_progress_close
        )
    ...
    def btn_update_click_main(self):
        ...
        ret = messagebox.askokcancel(
            "確認", "設定の更新のために、サーバーを再起動してもよろしいですか？"
        )
        if not ret:
            return
        ...
        try:
            try:
                ...
                self.progress_str_list[0].set(
                    self.progress_str_list[0].get() + "…エラー"
                )
                return
            ...
            try:
                ...
                if is_process_running(self.startExePath):
                    ...
                    self.progress_str_list[1].set(
                        self.progress_str_list[1].get() + "…エラー"
                    )
                    return
            ...
            try:
                ...
                self.progress_str_list[2].set(
                    self.progress_str_list[2].get() + "…エラー"
                )
                return
            ...
            try:
                ...
                logger.exception(e)
                self.progress_str_list[3].set(
                    self.progress_str_list[3].get() + "…エラー"
                )
                return
        ...
        if flag_unsave:
            ...
            if ret is not None:
                if ret:
                    for num, key in enumerate(self.keys):
                        self.update_detail(
                            key, self.table_string_list[num + 1][1].get()
                        )
                    self.write_default()
                self.root.destroy()
        else:
    ...
    def change_entry_width_key(self, index):
        ...
        def inner(var, index_, mode):
            txtlen = self.Cl_Pal.get_east_asian_width_count(
                self.table_string_list[index][0].get()
            )
    ...
    def change_entry_width_detail(self, index):
        ...
        def inner(var, index_, mode):
            txtlen = self.Cl_Pal.get_east_asian_width_count(
                self.table_string_list[index][1].get()
            )
    ...
    def change_entry_width_value(self, index):
        ...
        def inner(var, index_, mode):
            txtlen = self.Cl_Pal.get_east_asian_width_count(
                self.table_string_list[index][2].get()
            )
    ...
    def guimain(self):
        """画面作成"""
        self.root = tk.Tk()
        self.root.title("ini設定ツール" + "  Ver:" + VERSION)
        self.style = ttk.Style()
        self.style.configure(
            ".", font=("", 12)
        )  # Ttk Widget のフォントの大きさを 12 に変更
        ...
        self.canvas.bind(
            ...
            lambda e: self.canvas.yview_scroll(
                int(-1 * (e.delta / abs(e.delta))), "units"
            ),
        )
        self.canvas.bind(
            "<Shift-MouseWheel>",
        ...
        self.fr2 = tk.Frame(
            self.canvas, padx=0, pady=0, highlightthickness=0
        )  # ,bg='#000000FFF')
        self.fr2.bind(
            "<Configure>",
        ...
        self.maxlen_key, self.maxlen_detail, self.maxlen_value = (
            self.Cl_Pal.get_maxlen()
        )
        for num, i in enumerate([["キー", "説明", "設定値"]] + self.readkeys()):
            ...
            string_key.set(i[0])
            string_key.trace_add(
                "write", self.change_entry_width_key(len(self.table_entry_list))
            )
            entry_key = ttk.Entry(
                self.fr2, width=self.maxlen_key + 2, textvariable=string_key
            )  # , validate="focusout", validatecommand=self.change_entry_width_key(len(self.table_entry_list)))
            ...
            string_detail.trace_add(
                "write", self.change_entry_width_detail(len(self.table_entry_list))
            )
            entry_detail = ttk.Entry(
                self.fr2, width=self.maxlen_detail + 2, textvariable=string_detail
            )  # , validate="focusout", validatecommand=self.change_entry_width_detail(len(self.table_entry_list)))
            ...
            string_value.trace_add(
                "write", self.change_entry_width_value(len(self.table_entry_list))
            )
            entry_value = ttk.Entry(
                self.fr2, width=self.maxlen_value + 2, textvariable=string_value
            )  # , validate="focusout", validatecommand=self.change_entry_width_value(len(self.table_entry_list)))
            ...
            # もとに戻すボタン
            if num != 0:
                btn_discard = ttk.Button(
                    self.fr2,
                    text="元に戻す",
                    width=10,
                    command=self.btn_discard_click(len(self.table_discard_list)),
                )
        ...
        btn_update = ttk.Button(
            self.fr3,
            text="設定を適用(サーバーが再起動します)",
            command=self.btn_update_click,
        )
        btn_update.pack(anchor="se", padx=10, pady=10, side=tk.RIGHT)
        btn_output = ttk.Button(
            self.fr3, text="設定ファイルの出力", command=self.btn_output_click
        )
